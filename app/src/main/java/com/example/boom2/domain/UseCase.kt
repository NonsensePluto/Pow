...
class StartGameUseCase @Inject constructor(
    ...
    operator fun invoke(settingsViewModel: SettingsViewModel): GameState {
        val teams = teamGenerator.generateTeam(settingsViewModel.countOfTeams.value ?: 2)
        val unGuessedWords = wordsManager.selectRandomWords(settingsViewModel.countOfWords.value ?: 30)
...
class GenerateDefaultTeamsUseCase @Inject constructor(
    ...
    operator fun invoke(settingsState: SettingsState) {
        teamGenerator.generateTeam(settingsState.countOfTeams)
    }
}